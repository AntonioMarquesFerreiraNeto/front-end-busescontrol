  ...
  submitEdit() {
    ...
    this.clienteService.UpdateClientePJ(data).subscribe({
      next: () => {
        ...
        this.clienteService.GetClientesAtivos(this.compartilhamento.getPaginaAtualCliente(), true).subscribe((itens) => this.compartilhamento.autualizarClientePj(itens.clienteList));
      },
      error: (error: HttpErrorResponse) => {
