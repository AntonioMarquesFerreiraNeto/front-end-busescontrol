...
export class GerirClienteComponent implements OnInit {
  ...
  constructor(public modal: NgbActiveModal, private datePipe: DatePipe, private clienteService: ClienteService, private mensagemService: MensagensService,
    private compartilhar: CompartilharListService) {
  ...
  ActionModal() {
    if (this.cliente.status == 0) {
      this.clienteService.InativarCliente(this.cliente.id!).subscribe({
        next: () => {
          this.clienteService.GetClientes(this.compartilhar.getPaginaAtualCliente(), true).subscribe((itens) => {
            if (!itens.clienteList.length && (this.compartilhar.getPaginaAtualCliente() - 1) != 0) {
              this.clienteService.GetClientes(this.compartilhar.getPaginaAtualCliente() - 1, true).subscribe((itens) => {
                this.compartilhar.setPaginaAtualCliente(this.compartilhar.getPaginaAtualCliente() -1);
                this.compartilhar.autualizarCliente(itens.clienteList);
                this.compartilhar.setTotPaginaCliente(itens.qtPaginas);
              });
            } else {
              this.compartilhar.autualizarCliente(itens.clienteList);
              this.compartilhar.setTotPaginaCliente(itens.qtPaginas);
      ...
      this.clienteService.AtivarCliente(this.cliente.id!).subscribe({
        next: () => {
          this.clienteService.GetClientesInativos(this.compartilhar.getPaginaAtualCliente(), true).subscribe((itens) => {
            if (!itens.clienteList.length && (this.compartilhar.getPaginaAtualCliente() - 1) != 0) {
              this.clienteService.GetClientesInativos(this.compartilhar.getPaginaAtualCliente() - 1, true).subscribe((itens) => {
                this.compartilhar.setPaginaAtualCliente(this.compartilhar.getPaginaAtualCliente() -1);
                this.compartilhar.autualizarCliente(itens.clienteList);
                this.compartilhar.setTotPaginaCliente(itens.qtPaginas);
              });
