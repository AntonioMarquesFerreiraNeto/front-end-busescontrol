      ...
      this.contratoService.InativarContrato(this.contrato.id).subscribe({
        next: () => {
          ...
          this.contratoService.GetContratosAtivos(this.compartilhamento.getPaginaAtualContrato(), true).subscribe(itens => {
            if (!itens.contractList.length && this.compartilhamento.getPaginaAtualContrato() - 1 != 0) {
              this.contratoService.GetContratosAtivos(this.compartilhamento.getPaginaAtualContrato() - 1, true).subscribe(itens => {
                this.compartilhamento.atualizarContrato(itens.contractList);
